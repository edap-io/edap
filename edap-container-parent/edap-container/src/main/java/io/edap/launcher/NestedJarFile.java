 ...
 */
public final class NestedJarFile implements java.io.Closeable {
    ...
    public NestedJarFile(java.io.File file) throws IOException {
        this(new java.io.RandomAccessFile(file, "r"), 0L, file.length(), file.getName());
    }
    ...
    public java.util.Set<String> entryNames() {
    ...
    private byte[] readEntryBytes(CentralDirectoryParser.EntryData e) throws IOException {
        ...
        if (e.method == ZipEntry.STORED) {
            ...
            java.io.ByteArrayOutputStream out = new java.io.ByteArrayOutputStream((int) e.usize);
    ...
    public InputStream getInputStream(String name) throws IOException {
        CentralDirectoryParser.EntryData e = entries.get(name);
        if (e == null) throw new IOException("Entry not found: " + name);
        long absDataStart = dataOffsetInParent + e.localHeaderOffset + parseLocalHeaderSkip(
                readRange(physicalFile, dataOffsetInParent + e.localHeaderOffset, 30));
        physicalFile.seek(absDataStart);
        InputStream raw = new java.io.InputStream() {
        ...
        if (e.method == ZipEntry.STORED) return raw;
