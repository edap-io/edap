 ...
 * 实现类由 {@code AppContext.generateHandler(targetIf, entry, bean, method)} 用 ASM 字节码生成
 * （见 §3.5.7），协议入口方法（handle / handle(msg)）字节码按 bean method 参数类型
 * 硬编码 cast + 直接 invokevirtual——热路径零反射。
 *
 * Shard 不再独立成 5 份 List 之一：分片亲和是每个 RouteEntry 的 {@code shard} 字段
 * （HttpRouteEntry / WsRouteEntry / ErpcRouteEntry / GrpcMethodEntry）；当 {@code entry.shard() == true}
 * 时，生成 Handler 内部用 ShardRegistry 按 shardKey 选实例，所有协议统一走 handle(req, resp) / handle(msg)
 * 入口，零特殊协议类型。
 *
 * 注意：io.edap.http.WSHandler 是 WS 连接级事件接口，不在 4 份路由表里——它由 ServiceWSHandler 处理。
 *
 * RouterHub 不做 RouteEntry → Handler 的解析、不感知 ClassLoader、不做反射。
 */
