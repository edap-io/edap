...
import static io.edap.mqtt.ControlPacketType.PUBCOMP_VALUE;
...
public class MqttEncoderPubCompTest {

    @Test
    public void testEncodePubCompV3x() throws IOException {
        V31Encoder v31Encoder = new V31Encoder();
        Random random = new Random();
        int fixedByteValue = PUBCOMP_VALUE << 4;
        int dup = 1;
        ...
        PubComp pubComp = new PubComp(fixedByteValue);
        pubComp.setPacketIdentifier(messageId);

        MqttWriter writer = new MqttWriter();
        v31Encoder.encode(writer, pubComp);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);

        ByteArrayOutputStream out = new ByteArrayOutputStream();
        out.write(PUBCOMP_VALUE << 4 | (dup & 0x1) << 3 | (qos.getValue() & 0x3) << 1 | retain & 0x1);
        out.write(2);
    ...
    @Test
    public void testEncodePubRelV5() throws IOException {
        ...
        MqttWriter writer = new MqttWriter();
        v5Encoder.encode(writer, pubComp);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);
