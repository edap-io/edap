...
import static io.edap.mqtt.ControlPacketType.PUBREL_VALUE;
...
public class MqttEncoderPubRelTest {

    @Test
    public void testEncodePubRelV3x() throws IOException {
        V31Encoder v31Encoder = new V31Encoder();
        Random random = new Random();
        int fixedByteValue = PUBREL_VALUE << 4;
        int dup = 1;
        ...
        PubRel pubRel = new PubRel(fixedByteValue);
        pubRel.setPacketIdentifier(messageId);

        MqttWriter writer = new MqttWriter();
        v31Encoder.encode(writer, pubRel);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);

        ByteArrayOutputStream out = new ByteArrayOutputStream();
        out.write(PUBREL_VALUE << 4 | (dup & 0x1) << 3 | (qos.getValue() & 0x3) << 1 | retain & 0x1);
        out.write(2);
    ...
    public void testEncodePubRelV5() throws IOException {
        ...
        MqttWriter writer = new MqttWriter();
        v5Encoder.encode(writer, pubRel);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);
