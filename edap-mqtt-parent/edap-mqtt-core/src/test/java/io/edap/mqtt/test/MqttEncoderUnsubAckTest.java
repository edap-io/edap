...
import static io.edap.mqtt.ControlPacketType.UNSUBACK_VALUE;
...
public class MqttEncoderUnsubAckTest {

    @Test
    public void testEncodeUnsubAckV3x() throws IOException {
        V31Encoder v31Encoder = new V31Encoder();
        Random random = new Random();
        int fixedByteValue = UNSUBACK_VALUE << 4;
        int dup = 1;
        ...
        UnsubAck subAck = new UnsubAck(fixedByteValue);
    ...
    @Test
    public void testEncodeUnsubAckV5() throws IOException {
        ...
        UnsubAck unsubAck = new UnsubAck(fixedByteValue);
        unsubAck.setPacketIdentifier(messageId);
        unsubAck.setReasonCodes(codes);
        ...
        MqttWriter writer = new MqttWriter();
        v5Encoder.encode(writer, unsubAck);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);
