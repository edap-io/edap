...
import static io.edap.mqtt.ControlPacketType.PUBREC_VALUE;
...
public class MqttEncoderPubRecTest {

    @Test
    public void testEncodePubRecV3x() throws IOException {
        V31Encoder v31Encoder = new V31Encoder();
        Random random = new Random();
        int fixedByteValue = PUBREC_VALUE << 4;
        int dup = 1;
        ...
        PubRec pubRec = new PubRec(fixedByteValue);
        pubRec.setPacketIdentifier(messageId);

        MqttWriter writer = new MqttWriter();
        v31Encoder.encode(writer, pubRec);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);

        ByteArrayOutputStream out = new ByteArrayOutputStream();
        out.write(PUBREC_VALUE << 4 | (dup & 0x1) << 3 | (qos.getValue() & 0x3) << 1 | retain & 0x1);
        out.write(2);
    ...
    @Test
    public void testEncodePubRecV5() throws IOException {
        ...
        MqttWriter writer = new MqttWriter();
        v5Encoder.encode(writer, pubRec);
        byte[] data = new byte[writer.getLength()];
        System.arraycopy(writer.getData(), writer.getStart(), data, 0, data.length);
