...
package io.edap.data.jdbc.jdbc.test.dao;
...
public class DemoIntIdEntityDao extends JdbcBaseEntityDao implements JdbcEntityDao<DemoIntId> {
    ...
    @Override
    public List<DemoIntId> query(String sql, QueryParam... params) throws Exception {
        try {
    ...
    @Override
    public List<DemoIntId> query(String sql, Object... params) throws Exception {
        try {
            ResultSet rs = execute(sql, params);
            if (rs == null) {
                return EMPTY_LIST;
    ...
    @Override
    public DemoIntId findById(Object id) throws Exception {
        if (id == null) {
            return null;
        }
        StatementSession session = getStatementSession();
    ...
    @Override
    public DemoIntId findOne(String sql) throws Exception {
        List<DemoIntId> demos = query(sql);
        if (CollectionUtils.isEmpty(demos)) {
            return null;
        }
        return demos.get(0);
    ...
    @Override
    public DemoIntId findOne(String sql, QueryParam...  params) throws Exception {
        List<DemoIntId> demos = query(sql, params);
        if (CollectionUtils.isEmpty(demos)) {
            return null;
        }
        return demos.get(0);
    ...
    @Override
    public DemoIntId findOne(String sql, Object... params) throws Exception {
        List<DemoIntId> demos = query(sql, params);
        if (CollectionUtils.isEmpty(demos)) {
            return null;
        }
    ...
    @Override
    public int updateById(DemoIntId entity) throws Exception {
        StatementSession session = getStatementSession();
        try {
